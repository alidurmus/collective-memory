...
class EnhancedQueryEngine(QueryEngine):
    ...
    def _apply_semantic_search(self, results: List[EnhancedSearchResult], query: EnhancedSearchQuery) -> List[EnhancedSearchResult]:
    ...
    def _apply_ai_scoring(self, results: List[EnhancedSearchResult], query: EnhancedSearchQuery) -> List[EnhancedSearchResult]:
        ...
        for result in results:
            try:
                ...
                result.context_relevance = self._calculate_context_relevance(result, query)
    ...
    def _calculate_popularity_score(self, result: EnhancedSearchResult) -> float:
        ...
        popular_extensions = {'.md': 0.8, '.txt': 0.6, '.py': 0.7, '.js': 0.6, '.json': 0.5}
        score += popular_extensions.get(result.file_extension.lower(), 0.3)
        
        return min(score, 1.0)
    
    def _calculate_context_relevance(self, result: EnhancedSearchResult, query: EnhancedSearchQuery) -> float:
    ...
    def _extract_contextual_information(self, results: List[EnhancedSearchResult], query: EnhancedSearchQuery) -> List[EnhancedSearchResult]:
        """Extract contextual information and snippets"""
        
        for result in results:
            try:
                # Extract better contextual snippets
                result.contextual_snippets = self._extract_relevant_snippets(result, query)
                
                # Extract entities if enabled
                if query.extract_entities:
                    result.detected_entities = self._extract_entities(result.content_preview)
                
                # Calculate readability score
                result.readability_score = self._calculate_readability(result.content_preview)
                
            except Exception as e:
                logging.warning(f"Context extraction failed for {result.file_name}: {e}")
        
        return results
    
    def _extract_relevant_snippets(self, result: EnhancedSearchResult, query: EnhancedSearchQuery) -> List[str]:
    ...
    def _calculate_readability(self, content: str) -> float:
        ...
        sentences = len(sent_tokenize(content)) if self.ml_available else len(content.split('.'))
        ...
        syllables = sum(max(1, len([c for c in word if c.lower() in 'aeiou'])) for word in content.split())
    ...
    def _sort_enhanced_results(self, results: List[EnhancedSearchResult], query: EnhancedSearchQuery) -> List[EnhancedSearchResult]:
