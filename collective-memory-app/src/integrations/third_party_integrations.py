        ...
        try:
            for result in search_results[:10]:  # Limit to 10 results
                page_data = {
                    ...
                    "properties": {
                        "Name": {
                            "title": [{"text": {"content": result.get('title', 'Unknown')}}]
                        },
                        "File Path": {
                            "rich_text": [{"text": {"content": result.get('file_path', '')}}]
                        },
                        "Score": {
                            "number": float(result.get('score', 0))
                        },
                        "Synced": {
...
class ObsidianIntegration(BaseIntegration):
    ...
    def sync_data(self, search_results: List[Dict]) -> bool:
        ...
        try:
            ...
            index_content += f"**Synced:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
...
class IntegrationManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.config_path = Path(config_path) if config_path else Path('.collective-memory/integrations.json')
        self.integrations = {}
        ...
        try:
            ...
            for name, config in configs.items():
                ...
                if integration_type in self.integration_types:
                    self.integrations[name] = self.integration_types[integration_type](config)
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        for name, integration in self.integrations.items():
            ...
            status[name] = {
                ...
                'config_valid': bool(integration.test_connection() if config.get('enabled') else True)
            }
