...
class ImportanceScorer:
    ...
    def calculate_importance(self, content: str, memory_type: str,
                           context: str = None, metadata: Dict = None) -> float:
        ...
        if metadata and 'access_count' in metadata:
            factors.frequency = self._calculate_frequency_score(metadata['access_count'])
        
        # 6. User interaction score
        factors.user_interaction = self._calculate_user_interaction_score(content, metadata)
        ...
        logger.debug(f"Importance score calculated: {final_score:.3f} for type: {memory_type}")
        return final_score
    ...
    def _calculate_frequency_score(self, access_count: int) -> float:
        ...
        frequency_score = math.log(access_count + 1) / math.log(100 + 1)  # Normalize to log(100)
    ...
    def calculate_network_centrality(self, memory_id: int, 
                                   memory_links: List[Dict]) -> float:
    ...
    def update_importance_based_on_usage(self, current_score: float,
                                       access_count: int,
                                       time_since_access: int) -> float:
    ...
    def get_scoring_explanation(self, content: str, memory_type: str,
        ...
        explanation = {
            ...
            'context_relevance_score': self._calculate_context_relevance(content, context),
            'keyword_bonus': self._calculate_keyword_bonus(content),
            'weights': self.weights,
            'final_score': self.calculate_importance(content, memory_type, context, metadata)
        }
        
        if metadata and 'created_at' in metadata:
            explanation['recency_score'] = self._calculate_recency_score(metadata['created_at'])
        
        if metadata and 'access_count' in metadata:
            explanation['frequency_score'] = self._calculate_frequency_score(metadata['access_count'])
        
        if memory_type == 'code':
            explanation['code_complexity_score'] = self._calculate_code_complexity(content)
        
        if memory_type == 'error':
            explanation['error_criticality_score'] = self._calculate_error_criticality(content)
        
        if memory_type == 'decision':
            explanation['decision_impact_score'] = self._calculate_decision_impact(content)
