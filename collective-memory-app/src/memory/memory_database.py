    ...
    def delete_memory(self, memory_id: int, soft_delete: bool = True) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            if success:
                self._log_event(
                    "memory_deleted",
                    {"memory_id": memory_id, "soft_delete": soft_delete},
                )
    ...
    def create_memory_link(
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                # Link already exists
                logger.warning(
                    f"Memory link already exists: {memory_id_1} -> {memory_id_2}"
                )
                return None
    ...
    def create_conversation_context(
        ...
        with self.get_connection() as conn:
            ...
            self._log_event(
                ...
                {"context_id": context_id, "cursor_session_id": cursor_session_id},
            )
...
class MemoryEvolutionEngine:
    ...
    def process_interaction(
            ...
            if action == "ADD":
                ...
                if existing_memory:
                    self.db.update_memory(
                        ...
                        content=fact["content"],
                        importance_score=fact.get(
                            "importance", existing_memory["importance_score"]
                        ),
                        evolution_reason="Updated from conversation",
                    )
                    actions_taken.append(
                        {"action": "UPDATE", "memory_id": existing_memory["id"]}
                    )
        ...
        return {"extracted_facts": extracted_facts, "actions_taken": actions_taken}
    ...
    def _extract_facts(self, user_input: str, ai_response: str) -> List[Dict]:
        ...
        if "```" in ai_response:
            facts.append(
                {"content": "Code snippet provided", "type": "code", "importance": 0.7}
            )
