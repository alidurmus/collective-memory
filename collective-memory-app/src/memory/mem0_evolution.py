...
class FactExtractor:
    ...
    def extract_facts(
        self, user_input: str, ai_response: str, context: Dict
    ) -> List[ExtractedFact]:
    ...
    def _extract_from_text(
        self, text: str, source: str, context: Dict
    ) -> List[ExtractedFact]:
...
class Mem0EvolutionEngine:
    ...
    def process_interaction(
        ...
        logger.info(
            f"Processed interaction: {len(facts)} facts, {len(actions_taken)} actions ({processing_time:.3f}s)"
        )
        ...
        try:
            if decision.action == EvolutionAction.ADD:
                ...
                success = self.memory_database.delete_memory(
                    memory_id=decision.existing_memory_id, soft_delete=True
                )
    ...
    def _update_statistics(
        self, facts: List[ExtractedFact], actions: List[Dict], processing_time: float
    ):
...
if __name__ == "__main__":
    ...
    for fact in result.facts_extracted:
        print(
            f"  - {fact.content} (type: {fact.fact_type.value}, importance: {fact.importance:.3f})"
        )
