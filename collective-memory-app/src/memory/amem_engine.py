...
class AMemEngine:
    ...
    def create_memory_node(self, content: str, node_type: MemoryNodeType = MemoryNodeType.ATOMIC,
                          importance: float = None, tags: Set[str] = None,
                          context: Dict = None, metadata: Dict = None) -> MemoryNode:
        ...
        with self.lock:
            ...
            logger.info(f"Created memory node: {node_id[:8]}... (importance: {importance:.3f})")
            return node
    ...
    def _calculate_node_importance(self, content: str, context: Dict) -> float:
        ...
        # Keyword importance
        important_keywords = ['critical', 'important', 'key', 'main', 'core', 'essential']
    ...
    def _auto_link_memory(self, new_node: MemoryNode):
        ...
        for candidate_id, similarity in candidates:
            if similarity >= self.linking_threshold:
                # Determine link type
                link_type = self._determine_link_type(new_node, self.memory_nodes[candidate_id])
                
                # Create bidirectional link
                self._create_memory_link(
                    new_node.id, candidate_id, link_type, 
                    strength=similarity, confidence=0.8
                )
    ...
    def _find_linking_candidates(self, node: MemoryNode) -> List[Tuple[str, float]]:
        ...
        for existing_id, existing_node in self.memory_nodes.items():
            if existing_id != node.id:
                ...
                similarity = self._calculate_similarity(node_features, existing_node.content)
    ...
    def _extract_features(self, content: str) -> Dict:
        ...
        features = {
            ...
            'has_command': any(word in content.lower() for word in ['create', 'build', 'fix', 'update'])
        }
        return features
    ...
    def _extract_concepts(self, content: str) -> Set[str]:
        ...
        # Technical concepts
        tech_concepts = [
            'algorithm', 'architecture', 'design', 'pattern', 'framework',
            'library', 'api', 'database', 'server', 'client', 'security',
        ...
        prog_concepts = [
            ...
            'loop', 'condition', 'exception', 'inheritance', 'polymorphism'
        ]
    ...
    def _calculate_similarity(self, features1: Dict, content2: str) -> float:
        ...
        concept_similarity = len(concepts1 & concepts2) / len(concepts1 | concepts2) if concepts1 | concepts2 else 0
        
        # Code element overlap
        code1 = features1['code_elements']
        code2 = features2['code_elements']
        code_similarity = len(code1 & code2) / len(code1 | code2) if code1 | code2 else 0
    ...
    def _determine_link_type(self, node1: MemoryNode, node2: MemoryNode) -> LinkType:
        ...
        if any(word in content1 for word in ['because', 'since', 'due to', 'caused by']):
            return LinkType.CAUSAL
        
        # Check for contradictory relationship
        if any(word in content1 for word in ['not', 'no', 'never', 'opposite', 'contrary']):
            return LinkType.CONTRADICTORY
        
        # Check for supportive relationship
        if any(word in content1 for word in ['also', 'additionally', 'furthermore', 'supports']):
            return LinkType.SUPPORTIVE
        
        # Check for hierarchical relationship
        if any(word in content1 for word in ['part of', 'component', 'includes', 'contains']):
            return LinkType.HIERARCHICAL
        
        # Check for temporal relationship
        if any(word in content1 for word in ['after', 'before', 'then', 'next', 'previous']):
            return LinkType.TEMPORAL
        
        # Default to semantic
        return LinkType.SEMANTIC
    
    def _create_memory_link(self, source_id: str, target_id: str, link_type: LinkType,
                           strength: float, confidence: float) -> MemoryLink:
    ...
    def _update_network_stats(self):
        ...
        if self.stats['total_memories'] > 0:
            self.stats['avg_connections'] = (self.stats['total_links'] / 2) / self.stats['total_memories']
    ...
    def _calculate_network_importance(self, node_id: str) -> float:
        ...
        degree_centrality = degree / (len(self.memory_graph) - 1) if len(self.memory_graph) > 1 else 0
        ...
        if len(self.memory_graph) < 1000:
            try:
                betweenness = nx.betweenness_centrality(self.memory_graph).get(node_id, 0)
            except:
                betweenness = 0
        else:
            betweenness = 0
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            'connected_components': nx.number_connected_components(self.memory_graph.to_undirected()),
            'clustering_coefficient': nx.average_clustering(self.memory_graph.to_undirected()) if len(self.memory_graph) > 0 else 0
        }
