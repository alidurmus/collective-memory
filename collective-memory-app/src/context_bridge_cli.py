...
# Local imports
try:
    from smart_context_bridge import SmartContextBridge, ContextBridgeConfig, ChatContext
except ImportError:
...
class ContextBridgeCLI:
        ...
        if args.conversation:
            ...
            if json_path.exists():
                ...
                if context:
                    ...
                    print(f"{Fore.YELLOW}⚠️ Conversation analiz edilemedi{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}❌ Conversation bulunamadı: {json_path}{Style.RESET_ALL}")
    ...
    def cmd_config(self, args):
        ...
        if args.json_path:
            config.json_chat_path = args.json_path
            changed = True
            print(f"{Fore.GREEN}✅ JSON Chat path güncellendi: {args.json_path}{Style.RESET_ALL}")
            
        if args.rules_path:
            config.cursor_rules_path = args.rules_path
            changed = True
            print(f"{Fore.GREEN}✅ Cursor Rules path güncellendi: {args.rules_path}{Style.RESET_ALL}")
            
        if args.min_score is not None:
            config.min_relevance_score = args.min_score
            changed = True
            print(f"{Fore.GREEN}✅ Min relevance score güncellendi: {args.min_score}{Style.RESET_ALL}")
            
        if args.max_conversations is not None:
            config.max_conversations_to_analyze = args.max_conversations
            changed = True
            print(f"{Fore.GREEN}✅ Max conversations güncellendi: {args.max_conversations}{Style.RESET_ALL}")
    ...
    def _create_sample_conversation(self):
        ...
        sample_file = json_path / f"test_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _simulate_chat_change(self):
        ...
        if json_files:
            ...
            print(f"{Fore.YELLOW}⚠️ Güncellenecek JSON dosyası bulunamadı{Style.RESET_ALL}")
...
Examples:
  ...
  %(prog)s test --create-sample # Test conversation oluştur
    ...
    start_parser.add_argument('--max-conversations', type=int, help='Maximum conversation sayısı')
    start_parser.add_argument('--daemon', action='store_true', help='Background modda çalıştır')
    ...
    # Analyze command
    analyze_parser = subparsers.add_parser('analyze', help='JSON Chat dosyalarını analiz et')
    analyze_parser.add_argument('--conversation', help='Belirli conversation ID analiz et')
    
    # Config command
    config_parser = subparsers.add_parser('config', help='Konfigürasyon yönetimi')
    config_parser.add_argument('--show', action='store_true', help='Mevcut konfigürasyonu göster')
    ...
    config_parser.add_argument('--min-score', type=float, help='Min relevance score ayarla')
    config_parser.add_argument('--max-conversations', type=int, help='Max conversation sayısı ayarla')
    config_parser.add_argument('--enable-generation', type=bool, help='Context generation aktif/pasif')
    
    # Test command
    test_parser = subparsers.add_parser('test', help='Test ve demo fonksiyonları')
    test_parser.add_argument('--create-sample', action='store_true', help='Örnek conversation oluştur')
    test_parser.add_argument('--simulate-change', action='store_true', help='Chat değişikliğini simule et')
