...
class JSONChatManager:
    ...
    def __init__(self, data_folder: str = None):
        ...
        print(f"{Fore.GREEN}📁 JSON Chat Manager initialized: {self.conversations_dir}{Style.RESET_ALL}")
    ...
    def _load_index(self) -> Dict[str, Any]:
        ...
        if self.index_file.exists():
            try:
                ...
                print(f"{Fore.YELLOW}⚠️  Index dosyası yüklenemedi: {e}{Style.RESET_ALL}")
    ...
    def _save_index(self):
        ...
        try:
            self.conversation_index["updated_at"] = datetime.now(timezone.utc).isoformat()
    ...
    def create_conversation(self, title: str, project_path: str = None, 
        ...
        print(f"{Fore.GREEN}✅ Yeni konuşma oluşturuldu: {title} ({conversation_id}){Style.RESET_ALL}")
    ...
    def load_conversation(self, conversation_id: str) -> Optional[ChatConversation]:
        ...
        try:
            ...
            print(f"{Fore.RED}❌ Konuşma yüklenemedi {conversation_id}: {e}{Style.RESET_ALL}")
            return None
    
    def search_conversations(self, query: str = None, project_path: str = None,
    ...
    def import_from_cursor(self, cursor_reader) -> int:
        ...
        try:
            ...
            for workspace_key, workspace_data in workspaces.items():
                ...
                chats = cursor_reader.extract_complete_chat_history(workspace_data['db_path'])
            ...
            print(f"{Fore.GREEN}✅ {imported_count} Cursor konuşması import edildi{Style.RESET_ALL}")
    ...
    def _update_index_for_conversation(self, conversation: ChatConversation):
        ...
        # Total count
        self.conversation_index["total_conversations"] = len(self.conversation_index["conversations"])
    ...
    def _import_cursor_chat(self, chat_data: Dict[str, Any], 
                          workspace_info: Dict[str, Any]) -> Optional[str]:
        """Cursor chat'ini import et"""
        try:
            # Conversation title oluştur
            title = f"Cursor Chat - {chat_data.get('type', 'unknown')}"
            if 'summary' in chat_data:
                title = chat_data['summary'][:50] + "..." if len(chat_data['summary']) > 50 else chat_data['summary']
            
            # Conversation oluştur
            conversation_id = self.create_conversation(
                title=title,
                project_path=workspace_info.get('path', 'unknown')
            )
