...
class EnhancedCursorDatabaseReader:
    ...
    def _get_cursor_db_paths(self) -> List[Path]:
        ...
        mac_path = Path.home() / "Library" / "Application Support" / "Cursor" / "User" / "workspaceStorage"
        if mac_path.exists():
        ...
        if os.name == 'nt':
            windows_paths = [
                ...
                Path(os.environ.get('LOCALAPPDATA', '')) / "Cursor" / "User" / "workspaceStorage"
            ]
            ...
            for workspace_dir in cursor_path.iterdir():
                if workspace_dir.is_dir():
                    ...
                    if db_file.exists():
                        ...
                        if workspace_info:
                            workspace_key = f"{workspace_info['name']}_{workspace_dir.name}"
    ...
    def _get_workspace_info(self, db_file: Path) -> Optional[Dict]:
        ...
        try:
            with sqlite3.connect(str(db_file)) as conn:
                ...
                cursor.execute("SELECT value FROM ItemTable WHERE key = 'workbench.panel.explorer'")
                explorer_result = cursor.fetchone()
                
                # Recently opened folders
                cursor.execute("SELECT value FROM ItemTable WHERE key LIKE '%recently.opened%'")
                            ...
                            if entries and len(entries) > 0:
                                ...
                                if 'folderUri' in first_entry:
                                    ...
                                    if 'path' in folder_uri:
                                        ...
                                        workspace_info['name'] = Path(workspace_path).name
    ...
    def _parse_enhanced_chat_data(self, key: str, value: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if 'messages' in data and isinstance(data['messages'], list):
                    ...
                    chat_entry['summary'] = self._create_conversation_summary(data['messages'])
                ...
                # Inline chat format
                    ...
                    chat_entry['summary'] = self._create_content_summary(data['content'])
                ...
                chat_entry['summary'] = str(data)[:200] + "..." if len(str(data)) > 200 else str(data)
    ...
    def get_project_chat_history(self, project_path: Path, limit: int = 20) -> List[Dict]:
        ...
        for workspace_key, workspace_data in workspace_dbs.items():
            ...
            if workspace_info['path']:
                ...
                if project_str in workspace_path_str or workspace_path_str in project_str:
                    score = len(set(project_str.split('/')) & set(workspace_path_str.split('/')))
                    if score > best_score:
                        best_score = score
