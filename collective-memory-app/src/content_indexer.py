...
class ContentIndexer:
    ...
    def __init__(self):
        ...
        if MARKDOWN_AVAILABLE:
            ...
            self.md = None
            print(
                f"{Fore.YELLOW}⚠️  Markdown library not available. Using basic text parsing.{Style.RESET_ALL}"
            )
        ...
        for i, line in enumerate(lines):
            for match in re.finditer(link_pattern, line):
                metadata["links"].append(
                    {"text": match.group(1), "url": match.group(2), "line": i + 1}
                )
        ...
        for i, line in enumerate(lines):
            for match in re.finditer(image_pattern, line):
                metadata["images"].append(
                    {"alt": match.group(1), "url": match.group(2), "line": i + 1}
                )

        # Tables (basit tespit)
        for i, line in enumerate(lines):
            if "|" in line and line.strip().startswith("|"):
                metadata["tables"].append({"line": i + 1, "content": line.strip()})
        ...
        for i, line in enumerate(lines):
            ...
            if (
                ...
                metadata["lists"].append(
                    {"type": "unordered", "content": stripped[2:], "line": i + 1}
                )
            ...
            for pattern, emp_type in emphasis_patterns:
                for match in re.finditer(pattern, line):
                    metadata["emphasis"].append(
                        {"type": emp_type, "text": match.group(1), "line": i + 1}
                    )
    ...
    def extract_keywords(
        self, content: str, min_length: int = 3, max_keywords: int = 100
    ) -> List[Tuple[str, int]]:
        ...
        # Kelimeleri ayır
        words = re.findall(
            r"\b[a-zA-ZçğıöşüÇĞIİÖŞÜ]{" + str(min_length) + ",}\b",
            clean_content.lower(),
        )
    ...
    def create_search_index(self, file_path: str, content: str) -> Dict:
        ...
        for link in analysis["metadata"]["links"]:
            searchable_items.append(
                {"type": "link_text", "content": link["text"], "weight": 2}
            )
...
"""
    ...
    print(
        f"{Fore.YELLOW}📝 Search Index Created: {len(search_index['searchable_items'])} items{Style.RESET_ALL}"
    )
