...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def _notify_change(self, event_type: str, src_path: str, dest_path: Optional[str] = None):
        """Değişiklik bildirimini yapar"""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        print(f"{Fore.BLUE}[{timestamp}] {Fore.GREEN}FILE {event_type.upper()}: {Fore.YELLOW}{src_path}{Style.RESET_ALL}")
        
        if dest_path:
            print(f"{Fore.BLUE}[{timestamp}] {Fore.GREEN}MOVED TO: {Fore.YELLOW}{dest_path}{Style.RESET_ALL}")
...
class DataFolderMonitor:
    ...
    def start_monitoring(self):
        ...
        if not self.data_path.exists():
            print(f"{Fore.RED}ERROR: Data path does not exist: {self.data_path}{Style.RESET_ALL}")
            return False
            
        print(f"{Fore.GREEN}🔍 Starting file monitoring for: {Fore.YELLOW}{self.data_path}{Style.RESET_ALL}")
...
def main():
    ...
    def file_change_callback(event_type: str, src_path: str, dest_path: Optional[str] = None):
