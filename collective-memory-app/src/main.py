...
class CollectiveMemory:
    ...
    def collect_context(self, project_path, user_request):
        ...
        if self.config["include_sources"]["cursor_rules"]:
            ...
            context_data["sources"]["rules"] = self.context_collector.fetch_rules(project_path)
        ...
        if self.config["include_sources"]["project_docs"]:
            ...
            context_data["sources"]["docs"] = self.context_collector.fetch_docs(project_path)
...
def main():
    ...
    parser.add_argument("--request", type=str, 
                       help="Manuel olarak bağlam isteği belirt")
    parser.add_argument("--version", action="version", version="%(prog)s 1.0.0")
    
    # Yeni query sistem komutları
    parser.add_argument("--query", "-q", action="store_true",
                       help="Query/arama modunu başlat")
    parser.add_argument("--interactive", "-i", action="store_true",
                       help="İnteraktif arama modu")
    parser.add_argument("--search", "-s", type=str,
                       help="Doğrudan arama yap")
    parser.add_argument("--monitor", "-m", action="store_true",
                       help="Dosya izleme modunu başlat")
    parser.add_argument("--index", action="store_true",
                       help="Tüm dosyaları yeniden indeksle")
    parser.add_argument("--stats", action="store_true",
                       help="Sistem istatistiklerini göster")
    parser.add_argument("--save-to", type=str,
                       help="Arama sonuçlarını belirtilen dosyaya kaydet")
