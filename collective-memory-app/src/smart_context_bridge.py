...
class JSONChatMonitor(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        print(f"{Fore.BLUE}📁 JSON Chat değişikliği tespit edildi: {file_path}{Style.RESET_ALL}")
    ...
    def on_created(self, event):
        ...
        if not event.is_directory and event.src_path.endswith('.json'):
            print(f"{Fore.GREEN}📝 Yeni JSON Chat oluşturuldu: {event.src_path}{Style.RESET_ALL}")
...
class ContextExtractor:
    ...
    def __init__(self):
        self.important_keywords = [
            'proje', 'geliştirme', 'modül', 'özellik', 'bug', 'hata',
            'database', 'model', 'api', 'frontend', 'backend', 'test',
            'kural', 'standart', 'framework', 'context7', 'django', 'react'
        ]
    ...
    def extract_context_from_conversation(self, conversation: Dict) -> Dict:
        ...
        try:
            ...
            context['technical_details'] = self._extract_technical_details(recent_messages)
    ...
    def _extract_technical_details(self, messages: List[Dict]) -> List[str]:
        ...
        technical = []
        tech_keywords = ['model', 'class', 'function', 'api', 'database', 'framework', 'library']
    ...
    def _extract_next_steps(self, messages: List[Dict]) -> List[str]:
        ...
        next_steps = []
        step_keywords = ['sonraki', 'şimdi', 'daha sonra', 'next', 'then', 'yapacak', 'geliştir']
...
class CursorRulesUpdater:
    ...
    def update_auto_context(self, contexts: List[ChatContext]) -> bool:
        ...
        try:
            ...
            print(f"{Fore.GREEN}✅ Auto context güncellendi: {auto_context_path}{Style.RESET_ALL}")
            return True
...
class SmartContextBridge:
    ...
    def start_monitoring(self) -> bool:
        ...
        try:
            ...
            if not json_chat_path.exists():
                ...
                print(f"{Fore.YELLOW}📁 JSON Chat dizini oluşturuldu: {json_chat_path}{Style.RESET_ALL}")
                
            print(f"{Fore.CYAN}🚀 Smart Context Bridge başlatılıyor...{Style.RESET_ALL}")
            print(f"{Fore.BLUE}📁 İzlenen dizin: {json_chat_path.absolute()}{Style.RESET_ALL}")
            ...
            print(f"{Fore.YELLOW}💡 JSON Chat dosyalarınızda değişiklik yapın, otomatik context bridge oluşturulacak{Style.RESET_ALL}")
            
            return True
            
        except Exception as e:
            print(f"{Fore.RED}❌ Smart Context Bridge başlatma hatası: {e}{Style.RESET_ALL}")
            return False
    ...
    def _initial_context_analysis(self):
        ...
        print(f"{Fore.CYAN}🔍 Mevcut JSON Chat dosyaları analiz ediliyor...{Style.RESET_ALL}")
        
        json_chat_path = Path(self.config.json_chat_path)
        json_files = list(json_chat_path.glob("**/*.json"))
        
        if not json_files:
            print(f"{Fore.YELLOW}📝 Henüz JSON Chat dosyası bulunamadı{Style.RESET_ALL}")
            return
        ...
        if contexts:
            ...
            print(f"{Fore.GREEN}📊 {len(contexts)} konuşma analiz edildi{Style.RESET_ALL}")
        else:
            print(f"{Fore.YELLOW}⚠️ Analiz edilebilir konuşma bulunamadı{Style.RESET_ALL}")
            
    def _on_json_change(self, event_type: str, file_path: str):
        """JSON dosyası değiştiğinde tetiklenen callback"""
        print(f"{Fore.BLUE}🔄 JSON Chat değişikliği işleniyor: {event_type} - {file_path}{Style.RESET_ALL}")
    ...
    def _analyze_json_file(self, file_path: str) -> Optional[ChatContext]:
        ...
        try:
            ...
            # Context çıkar
            extracted_context = self.context_extractor.extract_context_from_conversation(conversation)
            ...
            print(f"{Fore.RED}❌ JSON analiz hatası ({file_path}): {e}{Style.RESET_ALL}")
            return None
    ...
    def _update_contexts_cache(self, new_context: ChatContext):
        ...
        if len(self.contexts_cache) > self.config.max_conversations_to_analyze:
            ...
            self.contexts_cache = self.contexts_cache[:self.config.max_conversations_to_analyze]
    ...
    def run_forever(self):
        ...
        try:
            print(f"{Fore.YELLOW}🎯 Smart Context Bridge çalışıyor... (Ctrl+C ile çıkış){Style.RESET_ALL}")
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'high_relevance_count': len([c for c in self.contexts_cache if c.relevance_score > 0.7]),
...
def main():
    ...
    print(f"{Fore.YELLOW}3. .cursor/rules/auto_context.md dosyasını kontrol edin{Style.RESET_ALL}")
    print()
