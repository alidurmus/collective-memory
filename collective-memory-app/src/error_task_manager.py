...
class ErrorTaskManager:
    ...
    def get_errors(self, status: str = None) -> List[Dict]:
        ...
        try:
            ...
            if status:
                cursor.execute(
                    "SELECT * FROM errors WHERE status = ? ORDER BY detected_at DESC",
                    (status,),
                )
            else:
    ...
    def update_error_status(
        self, error_code: str, status: str, resolution_notes: str = None
    ) -> bool:
    ...
    def update_task_status(
        self, task_code: str, status: str, progress: int = None, notes: str = None
    ) -> bool:
    ...
    def list_errors_console(self, status: str = None) -> None:
        ...
        for error in errors:
            ...
            severity_color = (
                Fore.RED if error["severity_level"] == "KRİTİK" else Fore.YELLOW
            )
            ...
            print(
                f"⚡ Önem: {severity_color}{error['severity_level']}{Style.RESET_ALL}"
            )
            ...
            if error["resolution_notes"]:
                print(
                    f"💡 Çözüm: {Fore.CYAN}{error['resolution_notes']}{Style.RESET_ALL}"
                )
    ...
    def list_tasks_console(self, status: str = None) -> None:
        ...
        for task in tasks:
            ...
            print(
                f"⚡ Öncelik: {priority_color}{task['priority_level']}{Style.RESET_ALL}"
            )
        ...
        while True:
            ...
            if command == "çıkış" or command == "quit" or command == "exit":
                ...
                if etm.update_error_status(error_code, "düzeltildi"):
                    print(
                        f"{Fore.GREEN}✅ Hata {error_code} düzeltildi olarak işaretlendi.{Style.RESET_ALL}"
                    )
                else:
                    print(
                        f"{Fore.RED}❌ Hata bulunamadı veya güncellenemedi.{Style.RESET_ALL}"
                    )
                ...
                if etm.update_task_status(task_code, "tamamlandı"):
                    print(
                        f"{Fore.GREEN}✅ Görev {task_code} tamamlandı olarak işaretlendi.{Style.RESET_ALL}"
                    )
                else:
                    print(
                        f"{Fore.RED}❌ Görev bulunamadı veya güncellenemedi.{Style.RESET_ALL}"
                    )
            else:
                print(
                    f"{Fore.YELLOW}❓ Bilinmeyen komut. Yardım için geçerli komutları yukarıda görebilirsiniz.{Style.RESET_ALL}"
                )
