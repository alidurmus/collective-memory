...
class ChatCLI:
    ...
    def create_conversation(self, title: str, project_path: str = None, 
    ...
    def list_conversations(self, limit: int = 20, project_path: str = None):
        ...
        try:
            ...
            print(f"{Fore.CYAN}📋 Konuşmalar ({len(conversations)} adet):{Style.RESET_ALL}\n")
    ...
    def show_conversation(self, conversation_id: str):
        ...
        try:
            ...
            if not conversation:
                print(f"{Fore.RED}❌ Konuşma bulunamadı: {conversation_id}{Style.RESET_ALL}")
                return
            ...
            print(f"{Fore.WHITE}Oluşturulma:{Style.RESET_ALL} {conversation.created_at}")
            print(f"{Fore.WHITE}Güncellenme:{Style.RESET_ALL} {conversation.updated_at}")
            print(f"{Fore.WHITE}Mesaj Sayısı:{Style.RESET_ALL} {len(conversation.messages)}")
            if conversation.tags:
                print(f"{Fore.WHITE}Etiketler:{Style.RESET_ALL} {', '.join(conversation.tags)}")
            ...
            for i, msg in enumerate(conversation.messages, 1):
                ...
                print(f"{role_color}{role_emoji} {msg.role.upper()}{Style.RESET_ALL} - {timestamp}")
                print(f"{msg.content}\n")
                print("-" * 80 + "\n")
    ...
    def search_conversations(self, query: str, limit: int = 10):
        ...
        try:
            ...
            if not conversations:
                print(f"{Fore.YELLOW}🔍 '{query}' için sonuç bulunamadı{Style.RESET_ALL}")
                return
            
            print(f"{Fore.CYAN}🔍 Arama Sonuçları '{query}' ({len(conversations)} adet):{Style.RESET_ALL}\n")
    ...
    def show_stats(self):
        ...
        try:
            ...
            print(f"{Fore.WHITE}Toplam Konuşma:{Style.RESET_ALL} {stats['total_conversations']}")
            print(f"{Fore.WHITE}Toplam Mesaj:{Style.RESET_ALL} {stats['total_messages']}")
            print(f"{Fore.WHITE}Storage Boyutu:{Style.RESET_ALL} {stats['storage_size']}")
            if stats['last_activity']:
                print(f"{Fore.WHITE}Son Aktivite:{Style.RESET_ALL} {stats['last_activity'][:19].replace('T', ' ')}")
    ...
    def import_from_cursor(self):
        ...
        try:
            ...
            print(f"{Fore.GREEN}✅ {imported_count} konuşma Cursor'dan import edildi{Style.RESET_ALL}")
...
Örnek Kullanımlar:
  ...
  python chat_cli.py import-cursor
    ...
    message_parser.add_argument('--role', default='user', choices=['user', 'assistant', 'system'])
    
    # List conversations
    list_parser = subparsers.add_parser('list', help='Konuşmaları listele')
    list_parser.add_argument('--limit', type=int, default=20, help='Maksimum sonuç sayısı')
    ...
    search_parser.add_argument('--limit', type=int, default=10, help='Maksimum sonuç sayısı')
