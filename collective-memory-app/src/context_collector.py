...
class ContextCollector:
    ...
    def __init__(self):
        ...
        self.supported_code_extensions = ['.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.cpp', '.c', '.h']
    ...
    def _load_memory_rules(self):
        ...
        try:
            ...
            cursor.execute('SELECT rule_type, rule_content FROM memory_rules WHERE is_active = TRUE')
    ...
    def _find_important_files(self, project_path: Path) -> List[Dict[str, Any]]:
        ...
        important_names = [
            ...
            'code_of_conduct', 'security', 'todo', 'notes'
        ]
        ...
        for dir_name in doc_dir_names:
            ...
            if doc_dir.exists() and doc_dir.is_dir():
                ...
                for doc_file in doc_dir.rglob('*'):
                    if doc_file.is_file() and doc_file.suffix in self.supported_doc_extensions:
                        try:
                            with open(doc_file, 'r', encoding='utf-8') as f:
                                ...
                                if content.strip():
                                    doc_files.append({
                                        'type': 'documentation',
                                        'name': doc_file.name,
                                        'content': content[:1500],  # İlk 1500 karakter
                                        'source': str(doc_file)
                                    })
                        except Exception as e:
    ...
    def collect_project_context(self, project_path: Path, cursor_reader) -> Dict[str, Any]:
