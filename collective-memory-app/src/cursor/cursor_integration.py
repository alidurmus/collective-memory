...
class CursorDatabaseReader:
    ...
    def __init__(self):
        ...
        if self.base_path and os.path.exists(self.base_path):
            self.workspace_storage_path = os.path.join(
                self.base_path, "workspaceStorage"
            )
...
class CursorMonitor(FileSystemEventHandler):
    ...
    def start_monitoring(self):
        ...
        try:
            self.observer.schedule(
                self, self.reader.workspace_storage_path, recursive=True
            )
            self.observer.start()
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory and event.src_path.endswith(".vscdb"):
            ...
            try:
                conversations = self.reader.extract_conversations_from_db(
                    event.src_path
                )
...
class CursorAnalyzer:
    ...
    def _extract_facts_from_message(self, message: Dict) -> List[Dict]:
        ...
        if role == "user":
            ...
            if "?" in content:
                facts.append(
                    {"type": "question", "content": content, "importance": 0.6}
                )

            # User instructions
            if any(
                word in content.lower()
                for word in ["create", "add", "fix", "change", "update"]
            ):
                facts.append(
                    {"type": "instruction", "content": content, "importance": 0.8}
                )
            ...
            if "explain" in content.lower() or "because" in content.lower():
                facts.append(
                    {"type": "explanation", "content": content, "importance": 0.7}
                )

            # Solutions provided
            if message.get("code_blocks"):
                facts.append(
                    {"type": "solution", "content": content, "importance": 0.9}
                )
...
class CursorIntegrationManager:
    ...
    def analyze_project_context(self, project_path: str) -> Dict:
        ...
        project_conversations = [
            conv for conv in conversations if conv.project_path == project_path
        ]
