...
def test_chat_history(cursor_reader, workspaces):
    ...
    if not workspaces:
        print(f"{Fore.YELLOW}⚠️  No workspaces available for chat testing{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        if chats:
            ...
            print(f"{Fore.YELLOW}⚠️  No chat history found in this workspace{Style.RESET_ALL}")
...
def test_project_matching(cursor_reader):
    ...
    try:
        ...
        if chats:
            print(f"{Fore.GREEN}✅ Found {len(chats)} project-specific chats:{Style.RESET_ALL}")
...
def main():
    ...
    print(f"   ✅ Cursor detection: {'Working' if cursor_reader.is_cursor_available() else 'Failed'}")
