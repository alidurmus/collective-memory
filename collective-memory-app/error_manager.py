...
def main():
    ...
    try:
        if command == 'hatalar':
            ...
            if len(sys.argv) < 3:
                print(f"{Fore.RED}❌ Hata ID'si gerekli! Örnek: python error_manager.py düzelt BACKEND_SERVER_DOWN{Style.RESET_ALL}")
                return
            
            error_id = sys.argv[2]
            if etm.update_error_status(error_id, 'düzeltildi'):
                print(f"{Fore.GREEN}✅ Hata {error_id} düzeltildi olarak işaretlendi{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}❌ Hata güncellenemedi: {error_id}{Style.RESET_ALL}")
                
        elif command == 'tamamla':
            if len(sys.argv) < 3:
                print(f"{Fore.RED}❌ Görev ID'si gerekli! Örnek: python error_manager.py tamamla TASK_001{Style.RESET_ALL}")
                return
            
            task_id = sys.argv[2]
            if etm.update_task_status(task_id, 'tamamlandı'):
                print(f"{Fore.GREEN}✅ Görev {task_id} tamamlandı olarak işaretlendi{Style.RESET_ALL}")
            else:
