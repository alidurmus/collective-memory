...
class ComprehensiveConsole:
    ...
    def __init__(self, db_path: str = "./comprehensive_system.db"):
        ...
        print(f"{Fore.CYAN}🚀 Comprehensive Console System Initialized{Style.RESET_ALL}")
    ...
    def run_interactive(self):
        ...
        while True:
            try:
                command = input(f"\n{Fore.GREEN}comprehensive> {Style.RESET_ALL}").strip()
                ...
                if cmd in self.commands:
                    try:
                        ...
                        self.log_action(f"COMMAND_EXECUTED", f"Command: {cmd}, Args: {args}")
                    except Exception as e:
                        print(f"{Fore.RED}❌ Command execution failed: {e}{Style.RESET_ALL}")
                        self.log_action(f"COMMAND_FAILED", f"Command: {cmd}, Error: {e}", False)
                else:
...
╚══════════════════════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
    ...
    def show_tasks(self, args: str = ""):
        ...
        try:
            ...
            for task in tasks:
                ...
                print(f"{status_color}[{task['status']}]{Style.RESET_ALL} "
    ...
    def add_task(self, args: str):
        ...
        try:
            ...
            for part in parts[1:]:
                if part.startswith('desc='):
                    ...
                    try:
                        ...
                        print(f"{Fore.YELLOW}⚠️ Invalid date format. Use YYYY-MM-DD{Style.RESET_ALL}")
            ...
            ''', (task_id, task_data['title'], task_data['description'], 
                  task_data['priority'], task_data['project'], task_data['due_date']))
    ...
    def delete_task(self, args: str):
        ...
        try:
            ...
            # Confirmation
            confirm = input(f"{Fore.YELLOW}⚠️ Delete task '{task['title']}'? (y/N): {Style.RESET_ALL}")
            if confirm.lower() != 'y':
    ...
    def search_tasks(self, args: str):
        ...
        try:
            ...
            if not results:
                print(f"{Fore.YELLOW}🔍 No tasks found matching '{args}'{Style.RESET_ALL}")
                return
    ...
    def generate_task_report(self, args: str = ""):
        ...
        try:
            ...
            cursor = self.connection.execute("SELECT status, COUNT(*) as count FROM tasks GROUP BY status")
            status_counts = {row['status']: row['count'] for row in cursor.fetchall()}
            
            cursor = self.connection.execute("SELECT priority, COUNT(*) as count FROM tasks GROUP BY priority")
    ...
    def show_errors(self, args: str = ""):
        ...
        try:
            ...
            for error in errors:
                ...
                print(f"{status_color}[{error['status']}]{Style.RESET_ALL} "
    ...
    def add_error(self, args: str):
        ...
        try:
            ...
            self.connection.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (error_id, error_data['title'], error_data['description'], 
                  error_data['error_type'], error_data['severity'], 
                  error_data['file_path'], error_data['stack_trace']))
    ...
    def search_errors(self, args: str):
        ...
        try:
            ...
            if not results:
                print(f"{Fore.YELLOW}🔍 No errors found matching '{args}'{Style.RESET_ALL}")
                return
    ...
    def generate_error_report(self, args: str = ""):
        ...
        try:
            ...
            cursor = self.connection.execute("SELECT status, COUNT(*) as count FROM errors GROUP BY status")
            status_counts = {row['status']: row['count'] for row in cursor.fetchall()}
            
            cursor = self.connection.execute("SELECT severity, COUNT(*) as count FROM errors GROUP BY severity")
            severity_counts = {row['severity']: row['count'] for row in cursor.fetchall()}
            
            cursor = self.connection.execute("SELECT error_type, COUNT(*) as count FROM errors GROUP BY error_type")
    ...
    def add_doc(self, args: str):
        ...
        try:
            ...
            self.connection.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (doc_id, doc_data['title'], doc_data['content'], 
                  doc_data['doc_type'], doc_data['category'], 
                  doc_data['author'], doc_data['version']))
    ...
    def delete_doc(self, args: str):
        ...
        try:
            ...
            # Confirmation
            confirm = input(f"{Fore.YELLOW}⚠️ Delete document '{doc['title']}'? (y/N): {Style.RESET_ALL}")
            if confirm.lower() != 'y':
                print(f"{Fore.CYAN}ℹ️ Operation cancelled{Style.RESET_ALL}")
                return
            
            self.connection.execute("DELETE FROM documentation WHERE id = ?", (doc['id'],))
            self.connection.commit()
            
            print(f"{Fore.GREEN}✅ Documentation deleted successfully!{Style.RESET_ALL}")
    ...
    def search_docs(self, args: str):
        ...
        try:
            ...
            if not results:
                print(f"{Fore.YELLOW}🔍 No documentation found matching '{args}'{Style.RESET_ALL}")
                return
    ...
    def export_doc(self, args: str):
        ...
        try:
            ...
            print(f"{Fore.GREEN}✅ Documentation exported successfully!{Style.RESET_ALL}")
    ...
    def generate_doc(self, args: str):
        ...
        try:
            if not args:
                print(f"{Fore.RED}❌ Please provide document type (system|api|user){Style.RESET_ALL}")
                return
            ...
            if doc_type == 'system':
                ...
                print(f"{Fore.RED}❌ Invalid document type: {doc_type}{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Available types: system, api, user{Style.RESET_ALL}")
    ...
    def show_system_status(self, args: str = ""):
        ...
        try:
            ...
            cursor = self.connection.execute("SELECT COUNT(*) as count FROM documentation")
            doc_count = cursor.fetchone()['count']
            
            cursor = self.connection.execute("SELECT COUNT(*) as count FROM system_logs")
            ...
            # Active tasks/errors
            cursor = self.connection.execute("SELECT COUNT(*) as count FROM tasks WHERE status != 'COMPLETED'")
            active_tasks = cursor.fetchone()['count']
            
            cursor = self.connection.execute("SELECT COUNT(*) as count FROM errors WHERE status != 'CLOSED'")
    ...
    def backup_system(self, args: str):
        """Sistem yedekle"""
        try:
            backup_name = args.strip() if args else f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            print(f"{Fore.GREEN}✅ System backup created successfully!{Style.RESET_ALL}")
    ...
    def restore_system(self, args: str):
        ...
        try:
            ...
            if not Path(backup_path).exists():
                print(f"{Fore.RED}❌ Backup file not found: {backup_path}{Style.RESET_ALL}")
                return
            
            # Confirmation
            confirm = input(f"{Fore.YELLOW}⚠️ Restore from backup '{backup_path}'? This will overwrite current data! (y/N): {Style.RESET_ALL}")
            if confirm.lower() != 'y':
                print(f"{Fore.CYAN}ℹ️ Operation cancelled{Style.RESET_ALL}")
                return
...
{Fore.YELLOW}📊 FILTERS:{Style.RESET_ALL}
  ...
  Doc Category: SYSTEM, USER, DEVELOPER, API
    ...
    def run_collective_memory(self, args: str):
        ...
        try:
            print(f"{Fore.CYAN}🚀 Starting Collective Memory System...{Style.RESET_ALL}")
            
            # Check if file exists
            if not Path("src/main.py").exists():
                print(f"{Fore.RED}❌ Collective Memory not found. Please run from correct directory.{Style.RESET_ALL}")
                return
            
            # Run collective memory
            cmd = ["python", "src/main.py"] + (args.split() if args else ["--interactive"])
            result = subprocess.run(cmd, capture_output=True, text=True)
            
            if result.returncode == 0:
                print(f"{Fore.GREEN}✅ Collective Memory completed successfully{Style.RESET_ALL}")
                print(result.stdout)
            else:
    ...
    def run_frontend(self, args: str):
        ...
        try:
            ...
            print(f"{Fore.GREEN}✅ Frontend Server started in background{Style.RESET_ALL}")
    ...
    def run_tests(self, args: str):
        ...
        try:
            ...
            cmd = ["python", "tests/test_runner.py"] + (args.split() if args else ["--all"])
            result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    def scan_system_errors(self, args: str):
        ...
        try:
            ...
            # Scan log files
            log_files = ["comprehensive_system.log", "collective_memory.log", "api_server.log"]
                        ...
                        for line_num, line in enumerate(f, 1):
                            if 'ERROR' in line or 'CRITICAL' in line:
                                ...
                                self.connection.execute('''
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?)
                                ''', (error_id, f"Log Error - Line {line_num}", line.strip(), 
                                      log_file, 'SYSTEM', 'MEDIUM'))
                                error_count += 1
            ...
            for file_path in required_files:
                if not Path(file_path).exists():
                    ...
                    self.connection.execute('''
                        INSERT INTO errors (id, title, description, file_path, error_type, severity)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (error_id, f"Missing File", f"Required file not found: {file_path}", 
                          file_path, 'SYSTEM', 'HIGH'))
                    error_count += 1
...
Run the system with: python comprehensive_console.py
            ...
            self.connection.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (doc_id, "System Documentation", content, "REFERENCE", "SYSTEM", "auto_generator"))
...
All responses are in JSON format with status, data, and meta fields.
            ...
            self.connection.execute('''
                INSERT INTO documentation (id, title, content, doc_type, category, author)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (doc_id, "API Documentation", api_content, "REFERENCE", "API", "auto_generator"))
            
            self.connection.commit()
            
            print(f"{Fore.GREEN}✅ Documentation auto-generated successfully{Style.RESET_ALL}")
            print(f"📚 Created 2 new documents")
            
        except Exception as e:
            print(f"{Fore.RED}❌ Error auto-generating documentation: {e}{Style.RESET_ALL}")
    ...
    def health_check(self, args: str):
        ...
        try:
            ...
            print(f"{Fore.GREEN if found_files == len(required_files) else Fore.YELLOW}📁 Files: {found_files}/{len(required_files)} found{Style.RESET_ALL}")
            ...
            for log_file in log_files:
                try:
                    ...
                    pass
            print(f"{Fore.GREEN if writable_logs == len(log_files) else Fore.YELLOW}📝 Logs: {writable_logs}/{len(log_files)} writable{Style.RESET_ALL}")
            ...
            try:
                ...
                if memory_percent < 80:
                    print(f"{Fore.GREEN}💾 Memory: {memory_percent}% used{Style.RESET_ALL}")
                    health_score += 20
                else:
                    print(f"{Fore.YELLOW}💾 Memory: {memory_percent}% used (high){Style.RESET_ALL}")
                    health_score += 10
            except ImportError:
                print(f"{Fore.YELLOW}💾 Memory: Cannot check (psutil not installed){Style.RESET_ALL}")
                health_score += 10
            ...
            if active_errors < 10:
                print(f"{Fore.GREEN}🐛 Active Errors: {active_errors} (good){Style.RESET_ALL}")
                health_score += 20
            else:
                print(f"{Fore.YELLOW}🐛 Active Errors: {active_errors} (needs attention){Style.RESET_ALL}")
                health_score += 10
    ...
    def cleanup_system(self, args: str):
        ...
        try:
            ...
            if old_tasks > 0:
                confirm = input(f"{Fore.YELLOW}⚠️ Remove {old_tasks} old completed tasks? (y/N): {Style.RESET_ALL}")
                if confirm.lower() == 'y':
                    self.connection.execute(
            ...
            if old_errors > 0:
                confirm = input(f"{Fore.YELLOW}⚠️ Remove {old_errors} old closed errors? (y/N): {Style.RESET_ALL}")
                if confirm.lower() == 'y':
                    self.connection.execute(
...
The system provides a command-line interface for all operations.
        ...
        self.connection.execute('''
            INSERT INTO documentation (id, title, content, doc_type, category, author)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (doc_id, "System Architecture", content, "REFERENCE", "SYSTEM", "auto_generator"))
