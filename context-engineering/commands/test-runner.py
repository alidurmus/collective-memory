...
class CollectiveMemoryTestRunner:
        ...
        try:
            ...
            if playwright_path.exists():
                ...
                result = subprocess.run([
                    "npx", "playwright", "test", 
                    "--config", str(self.app_root / "tests" / "playwright.config.js"),
                    "--reporter", "json"
                ], capture_output=True, text=True, cwd=str(self.app_root))
    ...
    def _test_turkish_characters(self):
        ...
        search_panel = self.app_root / "frontend" / "src" / "components" / "SearchPanel.jsx"
    ...
    def _test_turkish_date_format(self):
        ...
        if dashboard.exists():
            ...
            return "toLocaleString('tr-TR')" in content or "toLocaleDateString('tr-TR')" in content
        return False
        ...
        for test_name, test_func in perf_tests:
            try:
                ...
                if result["passed"]:
                    ...
                    suite_results["failed"] += 1
                    print(f"  ❌ {test_name}: {result['value']} (threshold: {result['threshold']})")
            except Exception as e:
                suite_results["failed"] += 1
                print(f"  ❌ {test_name}: {e}")
...
def main():
    ...
    parser.add_argument("--suite", help="Specific test suite to run", choices=["backend", "ui", "turkish", "performance", "context7"], default=None)
