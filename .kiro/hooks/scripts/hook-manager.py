...
class HookManager:
    ...
    def load_hooks(self):
        ...
        try:
            ...
            self.hooks = {hook['id']: hook for hook in config['hooks'] if hook.get('enabled', True)}
    ...
    def setup_file_watchers(self):
        ...
        file_save_hooks = [h for h in self.hooks.values() if h['trigger']['type'] == 'file_save']
    ...
    def setup_scheduler(self):
        ...
        scheduled_hooks = [h for h in self.hooks.values() if h['trigger']['type'] == 'scheduled']
        
        if scheduled_hooks:
            scheduler_thread = threading.Thread(target=self.run_scheduler, args=(scheduled_hooks,))
    ...
    def cleanup_action(self, config, context):
        ...
        print(f"✅ Temizlik tamamlandı: {deleted_count} dosya {'silinecek' if dry_run else 'silindi'}")
        ...
        for pattern in self.patterns:
            if file_path.match(pattern):
                ...
                for hook_id, hook in self.hook_manager.hooks.items():
                    if (hook['trigger']['type'] == 'file_save' and 
                        any(file_path.match(p) for p in hook['trigger'].get('patterns', []))):
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'start':
            ...
            for hook_id, hook in manager.hooks.items():
                print(f"  - {hook['name']} ({'aktif' if hook.get('enabled', True) else 'pasif'})")
        elif command == 'test':
